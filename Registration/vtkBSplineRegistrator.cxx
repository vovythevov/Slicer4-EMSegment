...
RegisterImagesInternal3()
{
  ...
  if (this->BulkTransform)
    ...
    for (int i = 0; i < 3; ++i)
      ...
      for (int j = 0; j < 3; ++j)
        {
        itkMatrix(i,j) = vtkMatrix->GetElement(i, j);
        }
      itkOffset[i] = vtkMatrix->GetElement(i, 3);
      }
...
#else
  this->Transform->SetDisplacementGridConnection(transformGenerator->GetOutputPort());
#endif
  std::cerr << "DONE" << std::endl;
