...
public:
  ...
  const char* mktemp_dir(const char* postfix);
...
std::string GetPreprocessingTasks();
  ...
  // set testFlag to 1 if you want to run it fast just for testing 
  vtkMRMLScalarVolumeNode* PreprocessingBiasFieldCorrection(vtkMRMLScalarVolumeNode *inputNode, int testFlag=0);
...
protected:
  ...
  ~vtkEMSegmentLogic();
   ...
   std::vector<double> IntensityRangeWithinMask(vtkImageData* image, vtkImageData* mask);
